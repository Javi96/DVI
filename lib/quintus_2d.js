...
var quintus2D = function(Quintus) {
    ...
    Quintus["2D"] = function(Q) {
         ...
         *
         * Propiedades:
         * * view_range         range of detection to trigger movement (number of tiles)
         * * attack_range       range of detection to trigger an attack (number of tiles)
         * * tile_size          size of a tile
         * * vfactor            the speed on x or y axis is incremented by tile_size*vfactor
         * * attacking          true when the ai is performing an attack
         * * tracking           true when the ai is tracking an enemy
         * * reloadSpeed        Speed the ai takes between attacks
         * * reload             current reload time
         * * track_class        Array of sprites to track
         *
         * @for Quintus.2d
         */
        Q.component('aiTrack', {
            ...
            range: function(sprites) {
                ...
                for (var i in sprites) {
                    for (var j in this.entity.p.track_class) {
                        if (sprites[i].isA(this.entity.p.track_class[j])) {
                            var other = sprites[i].p;
                            // Pythagoras to find out distance
                            var d = Math.sqrt((this.entity.p.x - other.x) * 2 + (this.entity.p.y - other.y) * 2) / this.entity.p.tile_size;
                            if (d <= this.entity.p.attack_range) {
                                this.entity.trigger('range.attack', sprites[i]);
                            } else if (d <= this.entity.p.view_range) {
            ...
            attack: function(obj) {
                if (!this.entity.p.damage || this.entity.p.reload > 0) return;
                this.entity.p.attacking = true;
                this.entity.p.reload = this.entity.p.reloadSpeed;
                obj.hit(this.entity.p.damage);
            },

            step: function(dt) {
                Q.stage().triggerRange();
